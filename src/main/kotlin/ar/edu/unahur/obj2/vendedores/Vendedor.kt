...
class ComercioCorresponsal(val ciudades: List<Ciudad>) : Vendedor() {
  ...
  //var provincias= mutableListOf<Provincia>()
  ...
  /*fun misProvincias(){
  this.ciudades.forEach { c -> if ( ! this.provincias.contains(c.provincia))
    this.provincias.add(c.provincia)}
}*/
  ...
  override fun esInfluyente()= ciudades.count() >= 5
          || ciudades.map { c -> c.provincia }.toSet().count() >= 3
